...
/** Aave adapter returns getUserHealthFactor payload `{ healthFactor, collected_at }` under `result.healthFactor`. */
function scalarHealthFactorFromPositionsResult(result) {
  const raw = result?.healthFactor;
  if (raw && typeof raw === "object" && "healthFactor" in raw) {
    return raw.healthFactor;
  }
  return raw ?? 0;
}
...
export async function getWalletPositions(userId, walletAddress) {
  ...
  for (const network of Object.values(networks)) {
    ...
    const healthFactor = scalarHealthFactorFromPositionsResult(result);
    if (result.error) {
