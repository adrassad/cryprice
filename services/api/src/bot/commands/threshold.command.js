...
import { patchMeByUserId } from "../../services/user/user.service.js";
...
export function thresholdCommand(bot) {
  ...
  bot.on("text", async (ctx, next) => {
    ...
    const internal = await getUserByTelegramId(ctx.from.id);
    if (!internal) {
      await ctx.reply(t(ctx.from.language_code, "profile.notFound"));
      return;
    }
    const user = await patchMeByUserId(internal.id, { threshold_hf: threshold });
